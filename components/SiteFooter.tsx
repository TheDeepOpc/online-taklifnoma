...
export function SiteFooter({ footer = DEFAULT_SITE_CONTENT.footer, contact = DEFAULT_SITE_CONTENT.contact }: { footer?: typeof DEFAULT_SITE_CONTENT.footer; contact?: typeof DEFAULT_SITE_CONTENT.contact }) {
  const socialLinks = [
    { label: contact.telegramLabel, href: contact.telegramHref, type: "telegram" as const },
    { label: contact.whatsappLabel, href: contact.whatsappHref, type: "whatsapp" as const },
  ];
    ...
    <footer className="border-t border-[#D4AF37]/15 bg-[#120d07] py-12 text-[#F7F2E9] sm:py-16">
      <div className="mx-auto max-w-6xl px-5 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4 sm:gap-10">
          ...
          <div className="sm:col-span-2 lg:col-span-1">
            <Link href="/" className="mb-4 flex items-center gap-2">
              ...
              <span className="font-display text-lg tracking-tight text-[#F7F2E9]">
                {footer.brand}<span className="text-[#D4AF37]">{footer.brandAccent}</span>{footer.brandSuffix}
              </span>
            </Link>
            <p className="max-w-[220px] text-sm leading-relaxed text-[#F7F2E9]/50">{footer.tagline}</p>
          </div>

          {/* Product */}
          <div>
            <h4 className="mb-3 text-sm font-semibold text-[#D4AF37]">{footer.productTitle}</h4>
          ...
          {/* Company */}
          <div>
            <h4 className="mb-3 text-sm font-semibold text-[#D4AF37]">{footer.companyTitle}</h4>
            <ul className="space-y-2.5">
          ...
          {/* Social */}
          <div>
            <h4 className="mb-3 text-sm font-semibold text-[#D4AF37]">{footer.socialTitle}</h4>
        ...
        <div className="mt-10 flex flex-col items-center gap-2 border-t border-[#D4AF37]/10 pt-6 sm:flex-row sm:justify-between">
          <p className="text-xs text-[#F7F2E9]/40">{footer.rights}</p>
          <p className="font-script text-lg text-[#D4AF37]/60">{footer.madeWith}</p>
        </div>
      </div>
    </footer>
  );
